...
### v5.1.0(revolution)
- **默认配置与磁贴**：增强全局默认配置能力，优化快捷设置磁贴启停链路；精简「建立配置」页中与默认配置相关的重复入口
- **悬浮球与自动关遮罩**：两条功能链路数据隔离，减少状态互相干扰；优化多配置组绑定同一包名时的切换与展示；配置弹窗支持分组标题样式
- **前台应用检测**：维护前台栈，优化需判断前台包名的场景（应用绑定、悬浮球显示等）
- **硬件快捷键**：新增「切换上一个 / 下一个遮罩」，可在绑定组内轮换配置；优化组合键冲突处理
- **Bug 修复**：修复应用退到后台后，通过悬浮球呼出的遮罩无法正确关闭的问题
